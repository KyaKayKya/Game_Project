...
SDL_Rect wireleft_and1 = { 438,457,80,5 };//andawire
SDL_Rect wireleft_and2 = { 591,458,160,5 };//andawire
...
// 根据元件类型和状态�?�置颜�??
    ...
    case WIRE:
        ...
        break;
    case AND_GATE:
        SDL_SetRenderDrawColor(renderer, 0x00, 0xFF, 0xFF, 0xFF); // AND
        break;
    case OR_GATE:
...
// 绘制�?�?
    ...
    // 绘制�?�?
    for (int i = 0; i < 2; i++) {
        renderComponent(andGates[i]); // 绘制AND�?
                                }

    for (int i = 0; i < 3; i++) {
        renderComponent(orGates[i]); // 绘制OR�?
                                }
                    ...
                    while (!bACk_tO_lASt_sCeNe_Puzz3)
                    {
                        ...
                        while (SDL_PollEvent(&e))
                        {
                            if (e.type == SDL_QUIT)
                            {
                                ...
                                SDL_SetRenderDrawColor(renderer, 0, 0, 0, 255); // 设置背景�?
                                                                                                                                SDL_RenderClear(renderer);
                                return 0;
                            }
